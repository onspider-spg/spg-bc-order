...
async function renderProductEditScreen() {
  ...
  // Build store+dept combos: only active ordering channels
  const storeVisRows = [];
  (S.orderingChannels || []).filter(ch => ch.is_active).forEach(ch => {
  ...
  storeVisRows.sort((a, b) => a.store_name.localeCompare(b.store_name) || a.dept_id.localeCompare(b.dept_id));
...
async function submitProductEdit(productId) {
  ...
  // Collect visibility
  // Collect visibility from active ordering channels only
  const visibility = [];
  (S.orderingChannels || []).filter(ch => ch.is_active).forEach(ch => {
